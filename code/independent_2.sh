...
/exdata/server/public_software_library/songjie_software/plink2 --bfile raw-GWA-data --maf 0.01 --geno 0.05 --hwe 0.00001 --make-bed --out clean-GWA-data

/exdata/server/public_software_library/songjie_software/plink2 --bfile clean-GWA-data --keep $1 --pheno $1  --pheno-col-nums 6 --glm no-firth no-x-sex --adjust --covar $2 --covar-col-nums 3-5 -out clean-GWA-data
...
cat res.glm.logistic.clean  | awk '$7<0.00001 {print}' > res.glm.logistic.clean.sig${chr}
/exdata/server/public_software_library/songjie_software/plink2 --bfile raw-GWA-data --keep $4 --pheno $4 --pheno-col-nums 6  --extract res.glm.logistic.clean.sig${chr} --export A  -out gwas/model_data${chr}.txt

mv res.glm.logistic.clean res.glm.logistic.clean.chr${chr}
...
mv clean-GWA-data_GWAS.chr* $3/

#cat res.glm.logistic.clean.chr* > $3/res.glm.logistic.clean
#cat res.glm.logistic.clean.chr*  | awk '{if($7<0.0000001) print $0 }' > $3/res.glm.logistic.clean.sig
